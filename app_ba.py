...
def custom_login():
    ...
    if request.method == 'POST':
        ...
        if not identifier or not password:
            flash('Please enter both username/email/phone and password.', 'danger')
            ...
            try:
                ...
                cursor.execute(
                    """SELECT username, password_hash, 
                              can_create_Asset_Tracking_Industrial,
                              can_read_Asset_Tracking_Industrial,
                              can_update_Asset_Tracking_Industrial,
                              can_delete_Asset_Tracking_Industrial
                       FROM User 
                ...
                if user and bcrypt.checkpw(
                    ...
                    create = user['can_create_Asset_Tracking_Industrial']
                    read = user['can_read_Asset_Tracking_Industrial']
                    update = user['can_update_Asset_Tracking_Industrial']
                    delete = user['can_delete_Asset_Tracking_Industrial']
                    ...
                    if role in ['admin', 'manager']:
                        ...
                        session['IOMS_can_create_Asset_Tracking_Industrial']=create
                        session['IOMS_can_read_Asset_Tracking_Industrial']=read
                        session['IOMS_can_update_Asset_Tracking_Industrial']=update
                        session['IOMS_can_delete_Asset_Tracking_Industrial']=delete

                        print(f"User {user['username']} logged in via QR with role {role}")
                        return redirect(url_for('view_asset_details', asset_id=asset_id))
                    else:
                        flash('No access permission for asset details', 'danger')
                        return render_template('qr_login.html', asset_id=asset_id)
                else:
...
def view_assets():
    ...
    base_query = """
        ...
        FROM asset_management.it_assets ia
        LEFT JOIN asset_management.assets_users au 
        ON ia.asset_id = au.asset_id
        AND au.effective_date = (
            SELECT MAX(effective_date)
            FROM asset_management.assets_users
            WHERE asset_id = ia.asset_id
        )
        AND au.archieved = 'No'
        AND au.overall_archieved = 'No'
    """
    ...
    for asset in assets_list:
        ...
        if filter_type == "assigned":
            ...
            if (effective_date is None and end_date is None and confirmation_status in [0, 1, None]) or \
               (effective_date and end_date and confirmation_status == 1) or \
               (effective_date and not end_date and confirmation_status == 0):
                filtered_assets.append(asset)
...
def view_asset_details(asset_id):
    ...
    cursor.execute("""
        SELECT sd.service_id, sd.warranty_type, sd.ticket_id, sd.work_done, sd.next_service_date, sd.service_charge
    ...
    cursor.execute("""
        SELECT rt.ticket_id, rt.problem_description, rt.ticket_status
...
#     # Fetch the latest assigned_user from assets_users
#     conn = get_db_connection()
#     cursor = conn.cursor()
#     cursor.execute("""
#         SELECT assigned_user, employee_code, email 
#         FROM assets_users 
...
#     # used_by = asset_user['assigned_user'] if asset_user else None
#     used_by = asset_user[0] if asset_user else None
...
#             'created_at': created_at,
#             'remarks': remarks if remarks else 'None',
#             'used_by': used_by if used_by else 'Not Assigned'
#         }
    ...
    if request.method == 'POST':
        ...
        cursor.execute("""
            INSERT INTO raised_tickets (
                ...
                problem_description, ticket_status, remarks, archieved, overall_archieved
            ) VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s)
        """, (created_by, created_at, ticket_id, asset_id, raised_by, employee_code,
              problem_description, ticket_status, remarks, archieved, 'No'))
        conn.commit()
...
def edit_service(service_id):
    ...
    if request.method == 'POST':
        ...
        # Get removed bills from form (full paths)
        removed_bills = request.form.get('removed_bills', '').split(',') if request.form.get('removed_bills') else []

        # Remove specified bills from the existing list
...
#             # Remove files from multiple_service_bill_path
#             updated_multiple_bill_paths = [path for path in existing_multiple_bill_paths if path not in removed_bills]
#             # Add new uploads to multiple_service_bill_path
#             if service_bill_paths:
#                 updated_multiple_bill_paths.extend(service_bill_paths)
#             multiple_service_bill_path = ','.join(updated_multiple_bill_paths) if updated_multiple_bill_paths else None

#             # Remove files from service_bill_path if they match removed_bills
#             updated_service_bill_paths = [path for path in existing_service_bill_paths if path not in removed_bills]
#             service_bill_path = ','.join(updated_service_bill_paths) if updated_service_bill_paths else None
...
def check_and_send_alerts(get_db_connection):  # Pass db connection function as parameter
    ...
    cursor.execute("""
        SELECT id.asset_id, id.insurance_end, asset_management.product_name
        FROM insurance_details id
        JOIN it_assets it ON id.asset_id = asset_management.asset_id
    ...
    cursor.execute("""
        SELECT sd.asset_id, sd.next_service_date, asset_management.product_name
        FROM service_details sd
        JOIN it_assets it ON sd.asset_id = asset_management.asset_id
...
def it_dashboard():
    ...
    cursor.execute("""
        ...
        FROM asset_management.raised_tickets rt
        JOIN asset_management.it_assets ia ON ia.asset_id = rt.asset_id;
    """)
    ...
    for row in tickets_rows:
        ...
        if row[1] == "completed":
            ...
            cursor.execute("""
                ...
                FROM asset_management.assets_users au
                WHERE au.asset_id = %s
                AND au.effective_date = (
                    SELECT MAX(effective_date)
                    FROM asset_management.assets_users
...
if __name__ == '__main__':
    try:
        ...
        app.run(host='192.168.2.48', port=8000, debug=True, use_reloader=False)
    except Exception as e:
        print(f"Error: {e}")
